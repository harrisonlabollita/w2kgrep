...
class GREP(object):
    def __init__(self, filename=None, *args):
        self.filename = filename if filename is not None else glob.glob("*scf")[0]
    ...
    def grep(self):
        ...
        for search in self.searches:
            ...
            if "-last" in self.options or "--last" in self.options:
                ...
                for find in new_grep: print(find, end="")


def main():
    orig_command_line = sys.argv[1:]
    filename = orig_command_line[-1] if "." in orig_command_line[-1] else None
    command_line = orig_command_line[:-1] if filename is not None else orig_command_line
